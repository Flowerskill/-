...
def parse_args():
    ...
    parser.add_argument('--data_lst', type=str, default='../../Data/MindRecord/voc_val_sar3_lst.txt', help='list of data')
    parser.add_argument('--dst_path', type=str, default='../../Data/MindRecord/voc_val_sar3.mindrecord', help='save path of mindrecords')
...
if __name__ == '__main__':
    ...
    dst_dir = '/'.join(args.dst_path.split('/')[:-1])    #  dst_dir = '../../Data/Mindrecords/VOC2012'
    ...
    for l in lines:
        ...
        cnt += 1
        if cnt % 1000 == 0:
            writer.write_raw_data(data)  #将数据写入到内存之中
            print('number of samples written:', cnt)
            data = []

    if data:
        writer.write_raw_data(data)
    writer.commit()  # 将最终内存中的数据写入到磁盘。
    print('number of samples written:', cnt)
