...
# pytorch 分布式 SynchronizedBatchNorm2d
# pytorch 初始化 default='he_uniform'
# mindspore 初始化 default='xavier_uniform'
...
class ASPPPooling(nn.Cell):
    ...
    def construct(self, x):
        ...
        out = nn.AvgPool2d(kernel_size=15)(x)
        out = self.conv(out)
...
class Decoder(nn.Cell):
    def __init__(self, num_classes, use_batch_statistics):
    ...
    def construct(self, x, size, low_level_features):
...
class DeepLabV3Plus(nn.Cell):
    def __init__(self, phase='train', num_classes=21, output_stride=16, freeze_bn=False):
        ...
        self.aspp = ASPP([1, 6, 12, 18], phase, 2048, num_classes,
                         use_batch_statistics=use_batch_statistics)
        self.shape = P.Shape()
        self.decoder = Decoder(num_classes, use_batch_statistics)
    ...
    def construct(self, x):
        ...
        out = self.decoder(out, size, low_level_features)
        return out
