...
class SegDataset:
    ...
    def preprocess_(self, image, label):
        # BGR image
        image_out = cv2.imdecode(np.frombuffer(image, dtype=np.uint8), cv2.IMREAD_COLOR)     # (722, 723, 3) # 函数从指定的内存缓存中读取数据，并把数据转换(解码)成图像格式;主要用于从网络传输数据中恢复出图像。
        label_out = cv2.imdecode(np.frombuffer(label, dtype=np.uint8), cv2.IMREAD_GRAYSCALE) # (722, 723)
    ...
    def get_dataset(self, repeat=1):
        data_set = de.MindDataset(dataset_files=self.data_file, columns_list=["data", "label"],
        ...
        transform_op = vision.RandomColorAdjust(brightness=(0.5, 1),
                                                contrast=(0.4, 1),
                                                saturation=(0.3, 1))
        nomalization_op = vision.Normalize(mean=self.image_mean, std=self.image_std)  # 暂时注释
        compose_op = Compose([pvision.HWC2CHW()])
        transforms_list = [transform_op, nomalization_op, compose_op]  # 暂时注释
        #transforms_list = [transform_op, compose_op]
